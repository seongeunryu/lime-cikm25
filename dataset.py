...
class Train_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        history_index = train_behavior[1]                                   # 이 유저가 클릭한 뉴스기사들의 index
        sample_index = self.train_samples[index]                            # 1 + neg_num 후보뉴스들의 index
        news_freshness= self.train_freshness[index]                         # 1 + neg_num 후보뉴스들의 freshness
        news_user_topic_lifetime = self.train_user_topic_lifetime[index]    # 1 + neg_num 후보뉴스들의 user_topic_lifetime
