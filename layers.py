...
class CandidateAware_ClickedNewsAttention(nn.Module):
    ...
    def forward(self, clicked_news_embeddings, clicked_news_topic_embeddings, candidate_topic_embeddings, mask=None):
        ...
        attention weight:               [batch_size, max_history_num]                       (W)
        mask:                           [batch_size, max_history_num] (optional)
        
        Output
        clicked_news_embeddings:        [batch_size, max_history_num, news_embedding_dim]
        """
        # 추후 성능이 더 필요할 때 multi-head로 확장하는 방향도 고려
        # Projection
        Q = self.query_proj(candidate_topic_embeddings)                            # [batch_size, news_num, news_embedding_dim]
        K = self.key_proj(clicked_news_topic_embeddings)                           # [batch_size, max_history_num, news_embedding_dim]

        # Compute attention scores (batch matrix multiplication)
        attn_scores = torch.einsum("bnd,bhd->bnh", Q, K) / self.scale              # [batch_size, news_num, max_history_num]
        # == attn_scores = torch.bmm(Q, K.transpose(1, 2)) / self.scale
        if mask is not None:
            attn_scores = attn_scores.masked_fill(mask.unsqueeze(1) == 0, -1e9)

        attn_weights = F.softmax(attn_scores, dim=-1)                              # [batch_size, news_num, max_history_num]
        if self.log_weights:
            self.last_attn_weights = attn_weights.detach().cpu()

        # Attention-weighted clicked news embeddings
        attn_weights = attn_weights.mean(dim=1)                                    # [batch_size, max_history_num] reduce
        # Normalize (optional) 성능 결과 보고 좋은 쪽으로 사용
        attn_weights = F.softmax(attn_weights, dim=-1)  # [B, H]
        
        weighted_clicked = attn_weights.unsqueeze(-1) * clicked_news_embeddings    # [batch_size, max_history_num, news_embedding_dim]

        '''
        Residual connection 목적
         - Representation degradation 방지: attention weight이 sharp하게 특정 뉴스에만 치우칠 경우 일부 클릭뉴스의 정보가 손실될 수 있음
         - Optimization stability: residual 사용하면 gradient flow가 더 원활해짐 (in ResNet)
        But, 무조건 성능 향상이라고 장담하긴 어려워서 이후 실험적으로 확인 필요 (True or False)
        '''
        # Residual connection
        if self.use_residual_connection:
            output = weighted_clicked + clicked_news_embeddings
        else:
            output = weighted_clicked

        return output, attn_weights             # output: [batch_size, max_history_num, news_embedding_dim]
    


class CandidateAware_ClickedNewsAttention_(nn.Module):
    def __init__(self, config, news_encoder):
        super(CandidateAware_ClickedNewsAttention_, self).__init__()
        self.topic_embedding_dim = config.category_embedding_dim
        self.use_residual_connection = config.use_residual_connection
        self.news_embedding_dim = news_encoder.news_embedding_dim
    ...
    def forward(self, clicked_news_embeddings, clicked_news_topic_embeddings, candidate_topic_embeddings, mask=None):
        ...
        clicked_news_topic_embeddings:  [batch_size, max_history_num, topic_embedding_dim]  (K)
        candidate_topic_embeddings:     [batch_size, news_num, topic_embedding_dim]         (Q)
        attention weight:               [batch_size, news_num, max_history_num]             (W)
        mask:                           [batch_size, max_history_num] (optional)
        
        Output
        clicked_news_embeddings:        [batch_size, max_history_num, news_embedding_dim]
        """
        batch_size, max_history_num, _ = clicked_news_embeddings.size()
        news_num = candidate_topic_embeddings.size(1)
        ...
        # Compute attention scores (batch matrix multiplication)
        attn_scores = torch.bmm(Q, K.transpose(1, 2)) / self.scale                  # [batch_size, news_num, max_history_num]        
                                                                                    # == torch.einsum("bnd,bhd->bnh", Q, K) / self.scale

        if mask is not None:
            mask = mask.unsqueeze(1).expand(-1, news_num, -1)                       # [batch_size, news_num, max_history_num]
        ...
        # Attention-weighted clicked news embeddings
        attn_weights_transposed = attn_weights.transpose(1, 2)                      # [batch_size, max_history_num, news_num]
        weighted_clicked = clicked_news_embeddings * attn_weights_transposed.unsqueeze(-1)  
                                                                                    # [batch_size, max_history_num, news_num, news_embedding_dim]
        weighted_clicked = weighted_clicked.sum(dim=2)                              # [batch_size, max_history_num, news_embedding_dim]
